...
install_yay() {
  ...
  # CachyOS ships paru pre-installed and packages yay in its own repo, so try
  # pacman first and only fall back to building from the AUR.
  warn "yay not found — installing..."
  echo
  if sudo pacman -S --needed --noconfirm yay; then
    log "yay installed from repo."
    echo
    return
  fi
  warn "yay not in repos — building from the AUR..."
